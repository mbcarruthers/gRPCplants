...
build_protoc: ## build proto files
	...
	-Iplants/proto \
	--go_out=plants/proto \
	--go_opt=module=github.com/mbcarruthers/gRPCplants \
	--go-grpc_out=plants/proto \
	--go-grpc_opt=module=github.com/mbcarruthers/gRPCplants plants/proto/plants.proto

clean_protoc:
	rm plants/proto/*pb.go

build_server: ## Builds server in bin/plants
	go build -o ${BIN_DIR}/$(project)/${SERVER_BIN} ./$(project)/${SERVER_DIR}

build_client: ## Builds client in bin/plants
	go build -o ${BIN_DIR}/$(project)/${CLIENT_BIN} ./$(project)/${CLIENT_DIR}

docker_up: ## Start up CockroachDB with docker-compose in detached mode
	sudo docker-compose up -d

docker_down: ## Shut down CockroachDB with docker-compose
